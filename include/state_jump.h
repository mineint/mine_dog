...
private:   
    ...
    const double flight_time_1 = 0.2;   // 阶段一飞行时长
    const double flight_time_2 = 0.2;   // 阶段二飞行时长
    const double flight_time = 0.5;  // 飞行时长
    const double crouch_high = -0.16;   // 下蹲高度
    const double rise_high = -0.28;   // 起立高度
    const double flight_high = -0.23;   // 飞行高度
    const double jump_back = -0.00;   // 跳跃时的x方向

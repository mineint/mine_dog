...
void State_Trot::runState(){
    ...
    for (int leg = 0; leg < 4; ++leg) {
        ...
        // 左右腿分别管理
        Eigen::Vector3d starting_pDes_l(-_data->trot_long_l,
                                        0.096 - _data->trot_wide_l,
                                        _data->start_high);

        Eigen::Vector3d starting_pDes_r(-_data->trot_long_r,
                                        0.096 - _data->trot_wide_r, 
                                        _data->start_high);

        Eigen::Vector3d destination_pDes_l(_data->trot_long_l, 
                                        0.096 + _data->trot_wide_l,  
                                        _data->start_high);
                                        
        Eigen::Vector3d destination_pDes_r(_data->trot_long_r, 
                                        0.096 + _data->trot_wide_r, 
                                        _data->start_high);
    ...
    switch (phase)
    {
        ...
        nominal_pDes = Eigen::Vector3d(-support_pDes[0], support_pDes[1], _data->start_high);

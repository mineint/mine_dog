    ...
    double stance_time)
{
    ...
    rc_vx = std::clamp(rc_vx, -0.25, 0.25); // vx不要超过0.25
    rc_vy = std::clamp(rc_vy, -0.2, 0.2);   // vy不要超过0.2
...
)
{
    ...
    // 转换为髋关节坐标系下（只计算当前腿）
    _foot_positions_leg.row(leg_id) = _foot_positions_body.row(leg_id) - _hip_positions_body.row(leg_id) + _foot_offset.row(leg_id);
